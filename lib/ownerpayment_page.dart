...
class _OwnerPaymentPageState extends State<OwnerPaymentPage>
  ...
  late TabController _tabController; // selectedBookingIds remove
  ...
  Future<void> _requestWithdraw(String bookingId) async {
    // Owner শুধু request করবে, paid করবে না
    ...
    if (mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Withdrawal request sent")));
    }
  }
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            ...
            .where(
              'admin_paid',
              isEqualTo: true,
            ) // KEY: Admin pay করলেই show করবে
            .where('payout_status', isEqualTo: payoutFilter)
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    _buildSummaryItem(
                      'Available',
                      'Tk ${payoutFilter == 'pending' ? totalEarned.toInt() : 0}',
                    ),
                    _buildSummaryItem(
                      'Withdrawn',
                      'Tk ${payoutFilter == 'paid' ? totalEarned.toInt() : 0}',
                    ),
                  ],
                ),
              ),
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final roomNo = data['room_number'] ?? '';
                    final amount = ownerAmount(data);
                    final isPaid = data['payout_status'] == 'paid';
